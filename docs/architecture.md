...
仓库根 `VERSION` 是项目版本的规范来源。当前版本 `0.8.8` 的 tag 为 `v0.8.8`，Release 必须同时包含：

```text
vpsctl.sh
vpsctl-manifest.tsv
vpsctl-core-0.8.8.tar.gz
vpsctl-network-0.8.8.tar.gz
vpsctl-system-0.8.8.tar.gz
vpsctl-security-0.8.8.tar.gz
vpsctl-service-0.8.8.tar.gz
vpsctl-test-0.8.8.tar.gz
```
...
bundle<TAB>core<TAB>vpsctl-core-0.8.8.tar.gz<TAB>SHA256
bundle<TAB>network<TAB>vpsctl-network-0.8.8.tar.gz<TAB>SHA256
bundle<TAB>system<TAB>vpsctl-system-0.8.8.tar.gz<TAB>SHA256
bundle<TAB>security<TAB>vpsctl-security-0.8.8.tar.gz<TAB>SHA256
bundle<TAB>service<TAB>vpsctl-service-0.8.8.tar.gz<TAB>SHA256
bundle<TAB>test<TAB>vpsctl-test-0.8.8.tar.gz<TAB>SHA256
```
...
访问管理的 root 与密码登录策略支持通过 `ssh apply` 确认后直接应用，包含备份、配置校验、reload 和失败恢复；此路径保持端口与防火墙不变，不创建会话证明或待提交事务。端口变更保留跨会话事务：临时并存旧端口与候选端口，新的、符合目标登录策略的 SSH 会话提交一次性证明后才允许提交最终配置。显式使用 `ssh prepare` 修改登录策略仍遵循原事务流程。事务状态和备份属于运行数据，不得放入仓库；功能脚本必须在写入前拒绝无法安全归并的复杂 SSH 配置，且不能把当前仍存活的旧会话当作新路径验证结果。
