...
访问管理 UI 将账户、公钥和 SSH 事务分组。SSH 加固不会在一次交互中直接关闭旧入口：准备阶段并存旧端口与候选端口，界面提示用户另开一个符合目标登录策略的 SSH 会话执行验证，然后返回原会话用事务 ID 强确认提交；用户也可中止并恢复准备前状态。防火墙自动协同只在功能脚本能唯一识别受支持后端且规则可安全管理时提供，否则要求选择手工模式并明确展示需维护的端口。复杂 SSH 配置会在任何写入前停止，菜单不提供绕过开关。
...
- 访问管理当前只支持 systemd 上的 SSH 服务编排，不将代理功能已有的 OpenRC 支持外推到 SSH。用户、密码和公钥操作与 SSH 配置事务分开；SSH 事务需要新的 SSH 会话证明，准备、提交、中止和历史恢复均保留明确入口。
