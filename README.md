...
> 当前版本为 0.3.0，提供 BBR、DNS、IP 地址族偏好和 RFW 四个 `network` 入口，并由 `service proxy` 平级管理 Xray、sing-box、出口驱动的节点中转和 nftables 端口转发。网络和代理服务变更都可能中断远程连接，请先使用 `--dry-run` 并阅读对应恢复说明。
...
- 功能命令：提供 `network bbr`、`network dns`、`network ip-policy`、`network rfw` 和 `service proxy`；均处于 `experimental` 生命周期。
