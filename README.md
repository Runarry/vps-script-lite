...
代理菜单还提供 [sing-box DNS](docs/proxy-management.md#sing-box-dns)：默认使用系统 DNS 兼容模式，也可选择 UDP、TCP、DoT、DoH 或 Cloudflare DoH 预设。设置统一用于 sing-box 本机解析，保留节点 IP 策略，不修改系统 DNS；例如 `bash bin/vpsctl service proxy dns set --preset cloudflare-doh`。
