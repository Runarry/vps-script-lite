...
[[ "$install_output" == 'vpsctl 0.8.7' ]] || fail 'bootstrap did not enter the installed CLI'
...
printf '0.8.8\n' >"$NEXT_SOURCE/VERSION"
sed -i 's/^readonly VPSCTL_VERSION="0.8.7"$/readonly VPSCTL_VERSION="0.8.8"/' "$NEXT_SOURCE/bin/vpsctl"
...
VPSCTL_TEST_CURL_FAIL=1 PATH="$MOCK_BIN:$PATH" "$ENTRY" --version | grep -Fx 'vpsctl 0.8.8' >/dev/null ||
